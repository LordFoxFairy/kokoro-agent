...
| 进程隔离 | worker 只装配 signer private-key settings/provider；HTTP 只装配 public-ring settings/handler。两者使用不同配置类型、启动检查、readiness 状态和脱敏日志，HTTP 对象图不得包含 private path/bytes/provider                                   |
...
freshness cache，避免代理 cache 叠加撤销延迟。route 不重定向，完整 canonical response 必须小于等于 64 KiB；这与 IAM 固定的
2 秒总 timeout、64 KiB response limit、禁止 redirect、per-issuer single-flight、known-key 30 秒 freshness和 unknown-kid 最多一次
