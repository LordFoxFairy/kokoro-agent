...
AG-UI 的公共事件名和运行时 schema 由 Root 的 `contract/agui-v1.json` 和 `@ag-ui/core` 提供；本仓只消费 Root/Agent 自己的内部
Chat/Run contract，不复制一份 AG-UI Python 类型。这样 `@ag-ui/core` 只在 Web/BFF 的浏览器边界
