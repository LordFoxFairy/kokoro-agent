...
- 用户可见的 `chat_messages`、`chat_events` 持久化事实；Web-facing 查询、replay 与投影由 `kokoro-bff Chat application` 承接。
...
- BFF、Capability、Storage 等其他 owner 的 API schema、数据库模型和生成器。
...
```text
Agent v1 `LaunchRunRequest(feature_key, message_id, content, ExecutionIdentity)`
  ...
  -> Agent Chat HTTP query boundary -> BFF Chat module
```

调用方不提交 namespace、thread、Agent、Skill、MCP、工具或 graph 配方；本仓 API 允许的模型
...
也不伪装成业务 `Service`。数据访问通过 `domain/<context>/repositories.py` 的 repository port，技术实现通过
`infrastructure/` adapter 提供；canonical schema operator use case 位于 `application/schema.py`。
...
Agent v1 的 `LaunchRunRequest` 是本仓入口语义；当前 Redis worker 使用严格 internal
envelope adapter（把 HTTP 顶层 `message_id/content` 映射到 `input`）。两者不形成第二套业务契约。
