...
`ag_ui_submission_provider.py` accepts one bounded fixture document, constructs official
AG-UI Python events through `kokoro_agent.presentation.build_submission`, and emits the exact
Session-facing `kokoro-session-presentation-compatibility-input.v1` bundle. It is compatibility evidence only: it does not
