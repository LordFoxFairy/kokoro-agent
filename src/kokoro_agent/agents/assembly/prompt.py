...
class PresetNotFoundError(Exception):
    """具名 preset 无对应资产：装配点 fail-loud，由 supervisor 收口为 assembly_failed。

    不静默回退 default——回退会让用户拿到 general 人格却以为跑的是所选 preset。
    """

    def __init__(self, name: str) -> None:
        super().__init__(f"agent preset {name!r} has no persona asset")
        self.name = name


def resolve_system_prompt(runtime: RuntimeConfig, prompts: PromptLibrary, *, default: str) -> str:
    """prompt 两级取值：
    ① agent（preset）名资产（prompts/<agent>.md）——具名 preset 的出厂 prompt；
       名字带了却找不到资产 = fail-loud（PresetNotFoundError），不静默回退 default。
    ② 未带 agent 名——default，类型工厂声明的通用缺省（缺省缺席即 general）。
    """
    if runtime.agent is None:
        return default
    prompt = prompts.get(runtime.agent)
    if prompt is None:
        raise PresetNotFoundError(runtime.agent)
    return prompt
...
def build_system_prompt(request: RunRequest, deps: AssembleDeps, *, default: str) -> str:
    ...
    base = resolve_system_prompt(request.runtime, deps.prompts, default=default)
