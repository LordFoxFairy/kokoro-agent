"""会话内 persona 切换：模型自判调 switch_persona 换应答人格。

无契约变更：候选=本部署 personas 资产全集（PromptLibrary.names），移交=切 system prompt 轨
（active_persona 进 graph state/checkpoint），工具面/skills/MCP 快照不变（换人格不换授权）。
候选<=1 时不挂 switch_persona、不挂人格中间件——单人格链路透明。
"""
...
SWITCH_PERSONA_TOOL_NAME = "switch_persona"


def swarm_candidates(prompts: PromptLibrary) -> tuple[str, ...]:
    """本 run 的可切换人格全集（目录即配置）；<=1 不挂切换工具。"""
    return prompts.names()


class SwitchPersonaArgs(BaseModel):
...
def make_switch_persona_tool(candidates: Sequence[str]) -> StructuredTool:
    """switch_persona 工具（仅候选>1 时挂载）：切换同一图的应答人格。

    未知名→纠错文本 fail-closed（不改 active_persona、不炸 run，模型据此自纠）；
    合法名→Command 覆盖 active_persona（LastValue 落 checkpoint）+ 一条 ToolMessage 回执。
    ...
    async def switch_persona(
        ...
        **_injected: object,
        ...
        LOGGER.info("persona switch active_persona -> %s", agent_name)
    ...
    return StructuredTool(
        name=SWITCH_PERSONA_TOOL_NAME,
        description=(
            "把当前对话的主导权移交给另一具名人格（换的是应答人格/system prompt，"
            f"工具与技能授权不变）。候选：{listing}。判断当前任务更适合某个人格时才调用。"
        ),
        args_schema=SwitchPersonaArgs,
        coroutine=switch_persona,
    )
...
def swap_persona_prompt(
    ...
    *,
    ...
    if base is None or not current_prompt.startswith(initial_prompt):
        return None  # 防御：候选已在 switch_persona 校验，此处不该命中
    return render_skill_manifest(base, grants) + current_prompt[len(initial_prompt) :]


class SwarmPersonaMiddleware(AgentMiddleware):
    """按 graph state 的 active_persona 动态切 system prompt 轨。

    只在候选>1 时挂载。未切换（active_persona 未设或＝装配期 preset）→原样透传。
    ...
    async def awrap_model_call(
        ...
        new_prompt = swap_persona_prompt(
            ...
            active_name=request.state.get("active_persona"),
