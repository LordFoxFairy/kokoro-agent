...
标准装配管线：工具解析 → 守卫 → 子代理 → 上下文 → 图。music 等数据型入口同走本
管线（差异全在 wire 携带的 bundle）；需要专属编排的类型另立 <type>.py 工厂。
"""
...
class GeneralAgentFactory(AgentFactory):
    ...
    async def create(self, deps: AssembleDeps, request: RunRequest) -> AssembledAgent:
        ...
        tools: list[BaseTool] = list(resolve_tools(runtime.tools, core=self.core_tools))
