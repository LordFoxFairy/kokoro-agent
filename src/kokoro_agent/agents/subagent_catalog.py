...
_CUSTOM_PAYLOADS: TypeAdapter[list[_SubagentDefinition]] = TypeAdapter(
    list[_SubagentDefinition]
)

# 内建目录只收"带真实工具挂载的真能力"；prompt 由受管 Agent/Feature 声明提供。
...
def build_subagent_catalog(
    ...
    builtins = tuple(
        spec for spec in BUILT_IN_SUBAGENTS if spec.name in enabled_builtins
    )
