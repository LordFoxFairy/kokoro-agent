...
from kokoro_agent.execution.events import tool_resolution_event
...
from kokoro_agent.execution.run_agent import InvokableAgent, events_stream, invoke_once
...
def _no_approval_tools(_request: RunRequest) -> frozenset[str]:
    return frozenset()


def _no_trace(_request: RunRequest) -> RunnableConfig | None:
    return None
...
class RunSupervisor:
    ...
    def __init__(
        ...
        store: RunStateStore,
        approval_tool_names: ApprovalToolNames = _no_approval_tools,
        trace_factory: TraceFactory = _no_trace,
        ...
        self._subagent_source = subagent_source
    ...
    def _spawn_agent(
        ...
        task = asyncio.create_task(
            self._guarded(bus, agent, run_id, conversation_id, payload, approval_tool_names, trace)
        )
    ...
    async def _guarded(
        ...
        async with self._sem:
            ...
            await invoke_once(
                ...
                subagent_source=self._subagent_source,
                trace=trace,
