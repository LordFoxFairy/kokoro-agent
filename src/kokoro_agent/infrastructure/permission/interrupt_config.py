...
_ASK_USER_DECISIONS: list[DecisionType] = ["reject", "respond"]
...
def build_interrupt_on(mode: PermissionMode) -> dict[str, InterruptOnConfig]:
    ...
    if mode == "auto":
        return {"ask_user_question": InterruptOnConfig(allowed_decisions=_ASK_USER_DECISIONS)}
    ...
    configured["ask_user_question"] = InterruptOnConfig(allowed_decisions=_ASK_USER_DECISIONS)
    return configured
