# persistence — Agent 运行持久化（PostgreSQL）

- `repository.py`：Run claim、lease、control command、terminal、effect journal 等 Agent 运行事实。
...
Skill package 的本地 fixture adapter 留在 `skills/local_reader.py`（历史 fixture），不从 persistence 域重导出。
