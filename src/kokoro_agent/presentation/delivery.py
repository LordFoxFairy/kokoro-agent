...
class PresentationProviderStore(Protocol):
    ...
    async def presentation_provider_status(
        self, run_id: str | None, original_command: common_wire.CommandIdentityV2 | None
    ) -> wire.DeliveryStatus: ...
...
def _validate_command(command: common_wire.CommandIdentityV2) -> None:
    if (
        not re.fullmatch("[0-9a-f]{32}", command.command_id)
        or not command.idempotency_key
        or command.digest_algorithm
        != common_wire.COMMAND_DIGEST_ALGORITHM_V2_SHA256_COMMAND_ENVELOPE
        or (_HEX_SHA256.fullmatch(command.request_digest) is None)
    ):
        raise ValueError("PRESENTATION_COMMAND_IDENTITY_INVALID")


def _validate_status(
    status: wire.DeliveryStatus, *, run_id: str, producer: wire.ProducerFence
) -> wire.DeliveryStatus:
    ...
    if status.status_revision < 1 or status.status_digest != delivery_status_digest(
        status
    ):
...
def _validate_records(
    *,
    ...
    records: tuple[wire.DeliveryRecord, ...],
) -> None:
    expected_previous = previous_record_digest
    for offset, record in enumerate(records, start=1):
        expected_sequence = after_delivery_seq + offset
        if (
            record.previous_delivery_seq != expected_sequence - 1
            or record.delivery_seq != expected_sequence
            or (not _producer_equal(record.producer, producer))
            or (record.previous_record_digest != expected_previous)
            or (record.record_digest != delivery_record_digest(run_id, record))
        ):
...
class DeliveryService:
    """Own Presentation delivery authorization, paging, and admission semantics."""

    def __init__(self, store: PresentationProviderStore) -> None:
        self._store = store

    async def check_active(
        self, request: wire.CheckActiveRequest
    ...
    async def _authorize_producer(
        self, run_id: str, claimed: wire.ProducerFence
    ) -> wire.ProducerFence:
    ...
    async def pull_records(
        self, request: wire.PullRecordsRequest
        ...
        frozen = (
            request.snapshot_through_delivery_seq
            if request.HasField("snapshot_through_delivery_seq")
            else current_head
        )
        ...
        cursor_record = (
            await self._store.presentation_provider_record(
                request.run_id, request.after_delivery_seq
            )
            if request.after_delivery_seq > 0
            else None
        )
        if (request.after_delivery_seq > 0) != (cursor_record is not None):
            raise ValueError("PRESENTATION_CURSOR_INVALID")
        records = await self._store.pull_presentation_provider_records(
            request.run_id, request.after_delivery_seq, frozen, request.page_size
        )
        _validate_records(
            run_id=request.run_id,
            producer=producer,
            after_delivery_seq=request.after_delivery_seq,
            previous_record_digest=cursor_record.record_digest
            if cursor_record is not None
            else record_chain_genesis_digest(request.run_id, producer),
            records=records,
        )
        page_end = records[-1].delivery_seq if records else request.after_delivery_seq
        has_more = page_end < frozen
        status = _validate_status(
            await self._store.presentation_provider_status(request.run_id, None),
            run_id=request.run_id,
            producer=producer,
        )
        response = wire.PullRecordsResponse(
            run_id=request.run_id,
            producer=producer,
            page_after_delivery_seq=request.after_delivery_seq,
            snapshot_through_delivery_seq=frozen,
            records=records,
            has_more=has_more,
    ...
    async def acknowledge_admissions(
        ...
        if (
            effect.effect_digest_domain != ACK_EFFECT_DOMAIN
            or effect.effect_digest != effect_digest(effect, ACK_EFFECT_DOMAIN)
            or effect.expected_status_revision < 1
            or (not effect.receipts)
        ):
        ...
        for receipt in effect.receipts:
            if (
                receipt.previous_delivery_seq != expected
                or receipt.delivery_seq != expected + 1
            ):
        ...
        status = _validate_status(
            await self._store.acknowledge_presentation_provider_admissions(request),
            run_id=effect.run_id,
            producer=producer,
        )
        if (
            status.acknowledged_through_delivery_seq != effect.receipts[-1].delivery_seq
            or not status.HasField("last_command")
            or status.last_command != request.command
        ):
    ...
    async def quarantine_submission(
        ...
        effect = request.effect
        if (
            effect.effect_digest_domain != QUARANTINE_EFFECT_DOMAIN
            or effect.effect_digest != effect_digest(effect, QUARANTINE_EFFECT_DOMAIN)
            or effect.expected_status_revision < 1
            or (effect.delivery_seq != effect.expected_acknowledged_through + 1)
            or (effect.rejection_class != wire.REJECTION_CLASS_PERMANENT)
        ):
            raise ValueError("PRESENTATION_QUARANTINE_EFFECT_INVALID")
        producer = await self._authorize_producer(effect.run_id, effect.producer)
        status = _validate_status(
            await self._store.quarantine_presentation_provider_submission(request),
            run_id=effect.run_id,
            producer=producer,
        )
        if (
            not status.HasField("quarantine")
            or status.quarantine.delivery_seq != effect.delivery_seq
            or status.quarantine.record_ref != effect.record_ref
            or (status.quarantine.submission_ref != effect.submission_ref)
            or (not status.HasField("last_command"))
            or (status.last_command != request.command)
        ):
    ...
    async def get_delivery_status(
        self, request: wire.GetDeliveryStatusRequest
        ...
        return wire.GetDeliveryStatusResponse(
            status=_validate_status(
                status, run_id=status.run_id, producer=authoritative
            )
        )
