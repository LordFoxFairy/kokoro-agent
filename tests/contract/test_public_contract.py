...
_ALL_KINDS: list[tuple[str, dict[str, JsonValue]]] = [
    ...
    (
        "tool.invoked",
        {"segment_id": "seg", "tool_id": "t1", "name": "execute", "args": {}},
    ),
    (
        "tool.output.delta",
        {"segment_id": "seg", "tool_id": "t1", "name": "execute", "delta": "ln\n"},
    ),
    ...
    (
        ...
        {"segment_id": "seg", "subagent_id": "sub1", "text": "xy"},
    ),
    (
        "delivery.created",
        {
...
@pytest.mark.parametrize(
    ("kind", "payload"), _ALL_KINDS, ids=[k for k, _ in _ALL_KINDS]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        _envelope(
            "run.failed", {"code": "internal_error", "error_kind": "E", "message": ""}
        ),  # 空错误消息
        _envelope(
            "run.failed", {"code": "oops", "error_kind": "E", "message": "m"}
        ),  # 码不在闭集
        _envelope("run.failed", {"error_kind": "E", "message": "m"}),  # 缺失败码
        _envelope(
            "todo.updated", {"todos": [{"content": "x", "status": "later"}]}
        ),  # 非法 todo 态
        _envelope(
            "subagent.started",
            {
...
# --- 控制面（Feature-first：session + feature + trusted identity） ---
...
@pytest.mark.parametrize(
    ...
    [
        ...
        _request(runtime={"backend": "state"}),  # caller 不得提供 runtime 配方
        _control("run.cancel", extra_field=1),  # 信封污染
        {"kind": "run.pause", "run_id": "r1", "session_id": "s1"},  # 未知 kind
    ],
)
