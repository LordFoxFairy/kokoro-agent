...
def test_chat_model_stream_reasoning_emits_thinking_delta() -> None:
    ...
    out = project(_ev("on_chat_model_stream", data={"chunk": chunk}), SubagentAttribution())
...
def test_tool_start_subagent_started_registers_attribution() -> None:
    ...
    assert project(text_ev, attribution)[0].kind == "subagent.text.delta"
...
def test_tool_end_subagent_finished_clears_attribution() -> None:
    ...
    assert project(text_ev, attribution)[0].kind == "text.delta"


def test_unknown_event_returns_empty() -> None:
    assert project(_ev("on_chain_start"), SubagentAttribution()) == []
