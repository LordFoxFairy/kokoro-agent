...
def test_litellm_construction_shape() -> None:
    ...
    model = make_chat_model(config.model, ModelConfig(provider="litellm", name="gateway-alias"))
...
def test_litellm_thinking_maps_reasoning_effort() -> None:
    ...
    def effort_of(thinking: bool | None, effort: str | None) -> object:
        model = make_chat_model(
            config.model,
            ModelConfig(provider="litellm", name="gateway-alias", thinking=thinking, effort=effort),
        )
        assert isinstance(model, ChatOpenAI)
...
def test_litellm_requires_api_key_fail_loud() -> None:
    config = AppConfig.from_env({"KOKORO_LITELLM_BASE_URL": "https://gateway.example.com/v1"})
