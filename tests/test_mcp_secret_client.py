"""hub 凭据解析客户端规格：出站信任头 + 请求体形状 / 响应信封解析 / 非2xx·网络·畸形→失败 /
空句柄不发请求 / env 装配开关。以 httpx.MockTransport 驱动，无真 hub。"""
...
_BASE = "http://hub.internal:4600"


def _resolver() -> HubSecretResolver:
    return HubSecretResolver(
        HubSecretSettings.model_validate({"base_url": _BASE, "service_secret": "agent-shared-secret"})
    )
...
async def test_sends_trust_headers_body_and_parses_secrets(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert seen["url"] == f"{_BASE}/hub/runtime/mcp/secrets/resolve"
...
async def test_non_2xx_raises_without_leaking_body(monkeypatch: pytest.MonkeyPatch) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        # 404（跨 namespace/不存在，hub 全有或全无）：错误体不得进异常文本。
        return httpx.Response(404, json={"error": {"code": "hub.secret_not_found", "message": "leak-me"}})
...
def test_env_switch_requires_both_base_url_and_secret() -> None:
    ...
    assert hub_secret_resolver_from_env({"KOKORO_HUB_BASE_URL": _BASE}) is None
    assert hub_secret_resolver_from_env({"KOKORO_INTERNAL_SECRET_AGENT": "s"}) is None
    resolver = hub_secret_resolver_from_env(
        {"KOKORO_HUB_BASE_URL": _BASE, "KOKORO_INTERNAL_SECRET_AGENT": "s"}
    )
    assert isinstance(resolver, HubSecretResolver)
