...
async def test_cancel_running_emits_cancelled() -> None:
    ...
    await sup.dispatch(
        bus, _inbound({"kind": "run.cancel", "command_id": "dec_wire", "run_id": "r5"})
    )
    await _drain(sup)
...
async def test_cancel_unknown_run_dropped() -> None:
    ...
    await sup.dispatch(
        bus,
        _inbound({"kind": "run.cancel", "command_id": "dec_wire", "run_id": "gone"}),
    )
    assert bus.published == []
...
async def test_cancel_after_natural_completion_no_duplicate_terminal() -> None:
    ...
    await sup.dispatch(
        bus,
        _inbound({"kind": "run.cancel", "command_id": "dec_wire", "run_id": "rc1"}),
    )
...
async def test_cancel_after_pause_emits_cancelled() -> None:
    ...
    await sup.dispatch(
        bus,
        _inbound({"kind": "run.cancel", "command_id": "dec_wire", "run_id": "rc2"}),
    )
...
async def test_resume_after_cancel_blocked_by_terminal() -> None:
    ...
    await sup.dispatch(
        bus,
        _inbound({"kind": "run.cancel", "command_id": "dec_wire", "run_id": "rc4"}),
    )
    before = len(bus.published)
    agent.seen_payloads.clear()
...
async def test_builder_failure_emits_run_failed_once() -> None:
    ...
    await sup.dispatch(
        bus,
        _inbound({"kind": "run.cancel", "command_id": "dec_wire", "run_id": "rbf"}),
    )
    terminals = [
