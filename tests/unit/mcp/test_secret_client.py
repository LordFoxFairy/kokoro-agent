...
def _resolver() -> CapabilitySecretResolver:
    return CapabilitySecretResolver(
        CapabilitySecretSettings.model_validate(
            {"base_url": _BASE, "service_secret": "agent-shared-secret"}
        )
    )
...
async def test_sends_trust_headers_body_and_parses_secrets(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"data": {"secrets": {_HANDLE: "Bearer plain"}}, "requestId": "r1"},
        )
...
async def test_non_2xx_raises_without_leaking_body(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            404,
            json={
                "error": {"code": "capability.secret_not_found", "message": "leak-me"}
            },
        )
...
async def test_network_error_raises_secret_resolve_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
...
async def test_empty_handles_makes_no_request(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ) -> httpx.Response:  # pragma: no cover - 不应被调用
...
def test_env_switch_requires_both_base_url_and_secret() -> None:
    ...
    assert (
        capability_secret_resolver_from_env({"KOKORO_CAPABILITY_BASE_URL": _BASE})
        is None
    )
    assert (
        capability_secret_resolver_from_env({"KOKORO_INTERNAL_SECRET_AGENT": "s"})
        is None
    )
